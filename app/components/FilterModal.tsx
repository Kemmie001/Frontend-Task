...
export default function FilterModal({ openModal, setOpenModal, setFilters }: Props) {
  ...
  return (
    ...
    <div className={`modal__bg ${openModal ? "visible" : ''}`} onClick={() => delayClose(600)}>
      <div onClick={(e) => e.stopPropagation()}>
        <div className={`modal__inner md:w-[460px] ${closingModal ? 'invisible' : ''}`}>
          <div className="flex justify-between items-center text-primary font-xl font-bold mb-10">
            <h2>Filter</h2>
            <button className='hover:bg-[#eff1f680]' onClick={() => delayClose(600)}>
              <Image src={close} alt="sent icon" width={20} height={20} />
            </button>
          </div>
          <div className="flex justify-between">
            <button className={`btn-primary chip ${dateRange === 0 ? 'active' : ''}`} onClick={() => selectDateRange(0)}>
              Today
            </button>
            <button className={`btn-primary chip ${dateRange === 7 ? 'active' : ''}`} onClick={() => selectDateRange(7)}>
              Last 7 days
            </button>
            <button className={`btn-primary chip ${dateRange === 30 ? 'active' : ''}`} onClick={() => selectDateRange(30)}>
              This month
            </button>
            <button className={`btn-primary chip ${dateRange === 90 ? 'active' : ''}`} onClick={() => selectDateRange(90)}>
              Last 3 months
            </button>
          </div>
        <div className="my-6 grow">
          ...
          <div className="flex justify-between gap-[6px]">
            ...
            <div className="my-10">
              <label className='font-semibold text-base mb-2 block'>Transaction Type</label>
              <SelectOptions
                label="Select Transaction Type"
                value={transactionType}
                options={['withdrawal', 'deposit']}
                onChange={(option: string) => modifyArray('transactionType', option)} />
            </div>
            <div className="my-10">
              <label className='font-semibold text-base mb-2 block'>Transaction Status</label>
              <SelectOptions
                label="Select Transaction Status"
                value={transactionStatus}
                options={['successful', 'pending', 'failed']}
                onChange={(option: string) => modifyArray('transactionStatus', option)} />
            </div>
